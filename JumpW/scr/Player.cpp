...
void Player::resetJumptimerTimer()
{
...
void Player::updatePhysics()
{
	...
	{
		this->velocity.y += (1.0f * this->AppexGravity) + (JumpInput * -20);
	}
	else
	{
		this->velocity.y += (1.0f * this->gravity) + (JumpInput * -20);
	}
...
void Player::updateMovement()
{
	...
	//Left
	(sf::Keyboard::isKeyPressed(sf::Keyboard::Key::A)) ? MoveLeft = -1 : MoveLeft = 0;
		//this->move(-1.f, 0.f);
		//this->animState = PLAYER_ANIMATION_STATES::MOVING_LEFT;
		// 
	//Right
	(sf::Keyboard::isKeyPressed(sf::Keyboard::Key::D)) ? MoveRight = 1 : MoveRight = 0;
	...
	{
		if (JumpButtomCounter > 1)
		{
			return;
		}

		JumpInput = 1;
		if (JumpTimer.getElapsedTime().asSeconds() > .05f)
		{
			canJump = false;
			resetJumptimerTimer();
		}
	...
	this->move(MoveLeft + MoveRight, 0.f);
}
